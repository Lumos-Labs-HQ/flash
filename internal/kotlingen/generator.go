...
func (g *Generator) generateDB() error {
	...
	switch {
	case driver == "exposed" || driver == "":
		w.WriteString("import org.jetbrains.exposed.sql.Database\n\n")
		w.WriteString("class Queries(private val db: Database) {\n")
		w.WriteString("    companion object {\n")
		w.WriteString("        fun newq(db: Database): Queries = Queries(db)\n")
		w.WriteString("    }\n")
		w.WriteString("}\n")
	case driver == "jdbc":
		w.WriteString("import java.sql.Connection\n\n")
		w.WriteString("class Queries(private val conn: Connection) {\n")
		w.WriteString("    private val stmts = mutableMapOf<String, java.sql.PreparedStatement>()\n\n")
		w.WriteString("    companion object {\n")
		w.WriteString("        fun newq(conn: Connection): Queries = Queries(conn)\n")
		w.WriteString("    }\n\n")
	...
	case driver == "r2dbc":
		...
		w.WriteString("    }\n")
		w.WriteString("}\n")
	case provider == "scylla" || provider == "scylladb" || provider == "cassandra":
		w.WriteString("import com.datastax.oss.driver.api.core.CqlSession\n\n")
		w.WriteString("class Queries(private val session: CqlSession) {\n")
		w.WriteString("    companion object {\n")
		w.WriteString("        fun newq(session: CqlSession): Queries = Queries(session)\n")
		w.WriteString("    }\n")
		w.WriteString("}\n")
	default:
		w.WriteString("import java.sql.Connection\n\n")
	...
	return os.WriteFile(filepath.Join(g.Config.Gen.Kotlin.Out, "DB.kt"), []byte(w.String()), 0644)
}
...
func ktTypedGetter(colName, sqlType string) string {
	...
	case strings.Contains(sl, "json"):
		return fmt.Sprintf("rs.getString(\"%s\")", colName) // return raw JSON string; caller parses
	default:
...
func ktTypedSetter(idx int, paramName, sqlType string) string {
	...
	default:
		// enums, json, arrays, text — setString or setObject
		return fmt.Sprintf("stmt.setString(%d, %s.toString())", idx, paramName)
	}
}
	...
	case ":one":
		...
		if len(columns) == 0 {
			...
			for i, col := range columns {
				...
				w.WriteString(fmt.Sprintf("                %s%s\n", ktTypedGetter(col.Name, col.Type), comma))
			}
